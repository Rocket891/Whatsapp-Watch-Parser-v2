...
export class WatchMessageParser {
  ...
  private async loadReferenceDatabase(): Promise<void> {
    ...
    try {
      ...
      const references = await storage.getAllReferenceRecords();
  ...
  private async enrichWithReference(pid: string): Promise<{ brand?: string; family?: string; name?: string }> {
  ...
  private extractPrice(text: string): { amount: number; currency: string } | null {
    ...
    // Handle standalone large numbers (6+ digits)
