...
export default function GroupManagement() {
  ...
  // Fetch current config to get dynamic instance ID and access token
  ...
  const { data: groupsData, isLoading: groupsLoading, refetch: refetchGroups } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiRequest('/api/whatsapp/groups', {
        ...
        body: JSON.stringify({})  // Use server-side config instead of hardcoded values
      });
  ...
  const currentWhitelist: string[] = (whitelistData as any)?.whitelistedGroups || [];

  // Initialize selected groups with current whitelist and populate manual input
  useEffect(() => {
    if (currentWhitelist.length > 0) {
      setSelectedGroups(currentWhitelist);
      // Also populate the manual input field with current whitelist
      setManualGroupId(currentWhitelist.join('\n'));
    }
  }, [currentWhitelist]);
  ...
  const updateWhitelistMutation = useMutation({
    mutationFn: async (groupIds: string[]) => {
      const response = await apiRequest('/api/whatsapp/whitelist', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ whitelistedGroups: groupIds.join(',') })
      });
      return response;
    },
    onSuccess: () => {
      toast({
        title: "Success",
        description: "Whitelisted groups updated successfully - changes applied instantly!",
      });
  ...
  const handleManualSave = () => {
    // Parse manual input and apply changes
    const groupIds = manualGroupId
      .split(/[\n,]+/)
      .map(id => id.trim())
      .filter(id => id.length > 0);
    ...
    updateWhitelistMutation.mutate(groupIds);
  };
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              className="flex-1"
            >
              {updateWhitelistMutation.isPending ? "Saving..." : "Save & Apply"}
            </Button>
          </div>
        </div>
      </div>
