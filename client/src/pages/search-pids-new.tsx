...
export default function SearchPIDs() {
  ...
  const { data: searchResults, isLoading, refetch } = useQuery({
    queryKey: (() => {
      ...
      return [`/api/watch-listings/search?${params}`];
    })(),
    enabled: false
  });
  ...
  return (
    <div className="flex h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <Topbar title="Search PIDs" subtitle="Advanced search and filtering" />
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  />
                  {filters.durationValue && filters.durationUnit && (
                    <div className="mt-1 text-xs text-green-600 flex items-center gap-1">
                      <span className="w-2 h-2 bg-green-500 rounded-full"></span>
                      <span>Active: Showing records from {filters.durationValue} {filters.durationUnit} ago to now</span>
                    </div>
                  )}
                </div>
                <div>
                  ...
                  {!filters.durationValue && filters.durationUnit && (
                    <div className="mt-1 text-xs text-amber-600 flex items-center gap-1">
                      ...
                      <span>Please enter a duration value</span>
                    </div>
                  )}
                </div>
              </div>
                    ...
                    <TableBody>
                      {Object.entries(summary.pidCounts)
                        ...
                        .map(([pid, count]) => {
                          ...
                          const prices = pidListings.map((l: any) => l.price).filter((p: number) => p && p > 0);
          ...
          {showResults && searchResults && searchResults.listings && searchResults.listings.length > 0 ? (
            <Card>
                    ...
                    <Button 
                      onClick={async () => {
                        try {
                          ...
                          a.href = downloadUrl;
                          a.download = `watch-listings-${new Date().toISOString().split('T')[0]}.xlsx`;
              ...
              <CardContent>
                ...
                <div className="overflow-x-auto overflow-y-auto max-h-[400px] sm:max-h-[600px] border rounded-lg excel-scroll">
                              ...
                              <div className="flex items-center gap-2">
                                <div className="flex-1 flex items-center gap-2">
                                  <WhatsAppBadge
                                    ...
                                    onSendMessage={listing.senderNumber ? () => {
                                      handleSendMessage(listing);
                                    } : undefined}
                                    size="md"
                                  />
